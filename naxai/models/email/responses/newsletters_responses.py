"""
Newsletter response models for the Naxai SDK.

This module defines the data structures for responses from newsletter-related API operations,
including newsletter creation, scheduling, retrieval, and management.
"""
...
class CreateNewsletterResponse(BaseNewsletterResponse):
    ...
    Inherits all attributes from BaseNewsletterResponse:
        ...
        - source (Literal["html", "editor"]): The source format of the newsletter content
        - state (Literal["draft", "scheduled", "sent"]):
          Current state (typically "draft" for new newsletters)
    ...
    Note:
        ...
        - To send the newsletter, it must be updated with a scheduled_at timestamp
          or sent immediately
...
class ListNewsLettersResponse(BaseModel):
    ...
    Example:
        ...
        >>>         {response.pagination.total_items} total newsletters")
        >>> 
    ...
    Note:
        ...
        - The items list contains complete newsletter information as defined
          in BaseNewsletterResponse
        ...
        - Each newsletter in the list contains its unique ID, which can be used for further
          operations
...
class GetNewsletterResponse(BaseNewsletterResponse):
    ...
    Note:
        - This response provides the current state of a newsletter, including its content
          and configuration
        - The state field indicates where the newsletter is in its lifecycle
          (draft, scheduled, sent)
        - For newsletters with state="sent", the sent_at field contains the sending timestamp
        - For newsletters with state="scheduled", the scheduled_at field contains the future
          sending time
        - For newsletters with source="html", the body field contains the complete HTML content
        - For newsletters with source="editor", the body_design field contains the structured
          design data
