"""
Email template response models for the Naxai SDK.

This module defines the data structures for responses from template-related API operations,
including template creation, retrieval, sharing, and management for email content.
"""
...
class BaseResponse(BaseModel):
    ...
    Attributes:
        ...
        created_at (Optional[int]): 
            Timestamp when the template was created, in milliseconds since epoch.
            Mapped from JSON key 'createdAt'. May be None if not available.
        modified_at (Optional[int]): 
            Timestamp when the template was last modified, in milliseconds since epoch.
...
class SharedTemplatesBaseResponse(BaseModel):
    ...
    Attributes:
        ...
        tags (Optional[list[str]]): 
            List of tags associated with the shared template for categorization.
...
class CreateTemplateResponse(BaseResponse):
    ...
    Note:
        - The created_at and modified_at timestamps will typically be the same for newly
          created templates
...
class ListTemplatesResponse(BaseModel):
    ...
    Example:
        ...
        >>> print(f"Displaying {len(response.items)} of \
        >>>      {response.pagination.total_items} total templates")
        >>> 
        >>> # List all templates
...
class GetTemplateResponse(BaseResponse):
    """
    Model representing the response from retrieving a specific email template in the Naxai
    email system.
    ...
    Note:
        ...
        - For templates with source="editor", the body_design field contains the complete
          structured design
...
class UpdateTemplateResponse(BaseResponse):
    ...
    Inherits all attributes from BaseResponse:
        ...
        - body_design (Optional[object]): The structured design data
          (for source="editor", may be updated)
        - thumbnail (Optional[str]): URL or base64 data of a thumbnail image (may be updated)
        - created_at (Optional[int]): Timestamp when the template was created
        - modified_at (Optional[int]): Timestamp when the template was updated
    ...
    Example:
        ...
        ...     source="html",
        ...     body="<html><body><h1>Welcome!</h1><p>Thank you for joining our platform.</p>\
        ...          </body></html>",  # Updated content
    ...
    Note:
        ...
        - For templates with source="editor", the body_design field contains the updated
          structured design
...
class ListSharedTemplatesRespone(BaseModel):
    ...
    Example:
        ...
        >>> print(f"Showing page {response.pagination.page} of {response.pagination.total_pages}")
        >>> print(f"Displaying {len(response.items)} of \
        >>>         {response.pagination.total_items} total shared templates")
        >>> 
    ...
    Note:
        - Use pagination parameters when making API requests to navigate through large result sets
        - The items list contains shared template information as defined
          in SharedTemplatesBaseResponse
        - Each shared template in the list contains its unique ID, which can be used to retrieve
          full details
...
class GetSharedTemplateResponse(SharedTemplatesBaseResponse):
    """
    Model representing the response from retrieving a specific shared email template in the Naxai
    email system.
    
    This class extends SharedTemplatesBaseResponse to include the content of the shared template,
    providing comprehensive information about a shared template's configuration,
    ...
    Attributes:
        ...
        body_design (Optional[object]): 
            The structured design data for templates created with the visual editor.
            Only populated when the template uses the visual editor. 
            Mapped from JSON key 'bodyDesign'.
            May be None if using raw HTML or if not specified.
    ...
    Example:
        ...
        ...     preview="https://example.com/preview/stpl_123abc",
        ...     tags=["marketing", "newsletter"],
        ...     body="<html><body><h1>Marketing Newsletter</h1>\
        ...          <p>Your content here.</p></body></html>"
        ... )
