"""
Unit tests for the asynchronous InboundResource class.
"""
import pytest
...
class TestInboundResource:
    """Test suite for the asynchronous InboundResource class."""
    ...
    @pytest.mark.asyncio
    async def test_list_with_hour_grouping(self, inbound_resource, mock_client):
        ...
        response = await inbound_resource.list(
            group="hour",
    ...
    @pytest.mark.asyncio
    async def test_list_with_day_grouping(self, inbound_resource, mock_client):
    ...
    @pytest.mark.asyncio
    async def test_list_with_month_grouping(self, inbound_resource, mock_client):
    ...
    @pytest.mark.asyncio
    async def test_list_with_number_filter(self, inbound_resource, mock_client):
    ...
    async def test_list_validation_errors(self, inbound_resource, group, start_date, stop_date, error_msg):
        """Test validation errors for invalid parameters."""
        with pytest.raises(NaxaiValueError, match=error_msg):
            await inbound_resource.list(group=group, start_date=start_date, stop_date=stop_date) 